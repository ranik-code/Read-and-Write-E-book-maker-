...
db.collection('books').doc(bookID).collection('chapters').doc(chapterID).get().then((doc) => {
    document.getElementById('heading').innerHTML = doc.data().chapterName; 
});

db.collection('books').doc(bookID).collection('chapters').doc(chapterID).collection('chapterContent').get().then((snapshot) => {
...
function updateDoc() {
    db.collection('books').doc(bookID).collection('chapters').doc(chapterID).update({
        chapterName: chapName
    })
    .then(()=>{
        db.collection('books').doc(bookID).collection('chapters').doc(chapterID).collection('chapterContent').doc(chapContentID).update({
        content: chapContent
        })
    })
    .then(()=>{
...
function exitPage() {
  location.replace("mystories.html");
}